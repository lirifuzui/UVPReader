...
class Analysis:
    ...
    def cylinderGeom(self, radius: int | float, wall_coordinate: int | float | None = None,
        ...
        # 其中的outward， fixedTDX和传感器朝向以及是否固定有关， ignoreException和报错有关
        # 这些参数目前还没有用到
        ...
        if delta_y is not None:
            ...
            self.__coordinate_array, self.__temp_coords = self.__temp_coords, self.__coordinate_array
            self.__vel_data, self.__temp_vel = self.__temp_vel, self.__vel_data
            self.__delta_y = self.__radius * np.max(max_magnitude) / max_vel
            if self.__delta_y > self.__radius:
    ...
    # 定义管道的尺寸，和上面的那个函数互斥
    def pipeGeom(self, pipe_TDXangle, pipe_radius: int | float | None = None,
                 wall_coordinate: int | float | None = None, flow_freq: int | float | None = None):
        # 这里的角度是与半径方向的夹角
        # 这里的夹角是圆柱截面和传感器指向的夹角
        if pipe_TDXangle is None or pipe_radius is None or wall_coordinate is None or flow_freq is None:
            raise ValueError(
                "All the parameters pipe_TDXangle, pipe_radius, wall_coordinate and flow_freq are required!")
        self.__radius = pipe_radius
        self.__pipe_TDXangle = pipe_TDXangle
        self.__shear_freq = flow_freq
        self.__coordinate_array = np.abs((wall_coordinate - self.__coordinate_array) * np.cos(
            self.__pipe_TDXangle / 180 * np.pi) + pipe_radius)
        for i in range(self.__number_of_windows + 1):
    ...
    def fftInUSR(self, window_num: int = 1, derivative_smoother_factor: int = 11):
        ...
        phase_delay = np.angle(fft_result[target_indices, range(fft_result.shape[1])])
    ...
    def rheologyViscosity(self, min_viscosity: int | float = 150, max_viscosity: int | float = 10000,
        ...
        if self.__radius is None and self.__pipe_TDXangle is None:
        ...
        for window in range(self.__number_of_windows + 1):
            ...
            viscosity_limits = [min_viscosity, max_viscosity]
            visc_range = int((max_viscosity - min_viscosity) / 40)
            ...
            for coordinate_index in range(len(self.__coordinate_array)):
                ...
                for loop in range(loop_count):
                    alpha_min = self.__Alpha_Bessel(self.__radius, oscillation_frequency,
                                                    viscosity_limits[0],
                    ...
                    '''import matplotlib.pyplot as plt
                    plt.figure()
                    plt.plot(self.__coordinate_array, Tools.derivative(alpha_min, self.__coordinate_array))
                    plt.show()'''
                    # print(alpha_min_derivative)
                    # ---------------------------------------------------------------------------------------------------------------
                    alpha_max = self.__Alpha_Bessel(self.__radius, oscillation_frequency,
                                                    viscosity_limits[1],
                                                    self.__coordinate_array)
                    ...
                    alpha_middle = self.__Alpha_Bessel(self.__radius, oscillation_frequency,
                    ...
                    '''import matplotlib.pyplot as plt
                    plt.figure()
                    plt.plot(self.__coordinate_array, Tools.derivative(alpha_middle, self.__coordinate_array))
                    plt.show()'''
                    # print(alpha_middle_derivative)
                    # print(alpha_max_derivative)
                    # ---------------------------------------------------------------------------------------------------------------
                    ...
                    if idx == 1:
                        ...
                        print("\033[1m\033[31mCALCULATION ERROR：\033[0m" +
                              "Unable to find viscosity within defined range"
                              "(" + str(min_viscosity) + ', ' + str(max_viscosity) + ').')
                        effective_viscosity_for_now_window[coordinate_index] = -1
                        viscosity_limits = [min_viscosity, max_viscosity]
                        self.__err_time += 1
                        break
                    if np.abs(temp[0] - temp[1]) < viscosity_range_tolerance:
                        print(f'{window:<{slice_width}}{coordinate_index:<{index_width}}'
                              ...
                              f'{shear_rate_for_now_window[coordinate_index]:<{shear_rate_width}.5g}')
                        effective_viscosity_for_now_window[coordinate_index] = middle_viscosity
                        viscosity_limits = [
        ...
        self.__shear_rate = [self.__shear_rate[i] for i in range(len(self.__viscosity_cSt)) if
                             self.__viscosity_cSt[i] != -1]
        self.__viscosity_cSt = self.__viscosity_cSt[self.__viscosity_cSt != -1]
    ...
    @property
    def geometry(self):
        return self.__radius, self.__delta_y
