...
def unnamed(phase_delay_derivative, coordinate_series):
    ...
    for i in range(len(coordinate_series)):
        ...
        for cycle_index in range(50):
            ...
            if (predicted_value[0] - phase_delay_derivative) * (
                    phase_delay_derivative - predicted_value[1]) > 0:  # 如果值在预定的粘度最小值和粘度当前值之前
                if abs(copy_initial_visc[0] - copy_initial_visc[2]) < prec:
                    ...
                    copy_initial_visc[2] = copy_initial_visc[1]
                    copy_initial_visc[1] = (copy_initial_visc[0] + copy_initial_visc[1]) / 2
